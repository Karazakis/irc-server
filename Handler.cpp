...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/20 17:26:56 by akoutsou          #+#    #+#             */
/*   Updated: 2024/03/29 16:54:59 by akoutsou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int Handler::handleCommand(int client_fd, const std::vector<std::string>& cmdParams, std::map<int, Client*>& clients, std::map<std::string, Channel*>& channels)
{
    ...
    if (cmd == "CAP") {
        ...
        handleModeCommand(client_fd, cmdParams, channels, clients);
    } else if (cmd == "INVITE") {
...
void Handler::handleModeCommand(int client_fd, const std::vector<std::string>& cmdParams, std::map<std::string, Channel*>& channels, std::map<int, Client*>& clients) {
    ...
    if(cmdParams.size() > 2 )
    {
        ...
        else
        {
            ...
            {
                errorMesg = ":SOVIET 481 " + clients[client_fd]->getNickname() + " :Permission Denied - You are not a channel operator, beeetch\r\n";
                send(client_fd, errorMesg.c_str(), errorMesg.length(), 0);
            }
            else
            {
                errorMesg = ":SOVIET 421 " + channel_name + " :Unknown\r\n";
                send(client_fd, errorMesg.c_str(), errorMesg.length(), 0);
            }
        }
    }
...
void Handler::handleTopicCommand(int client_fd, const std::vector<std::string>& cmdParams, std::map<int, Client*>& clients, std::map<std::string, Channel*>& channels)
{
    ...
    if (channels.find(channelName) != channels.end()) {
        ...
        if (channel->isOperator(client_fd)) {
            ...
            {
                std::string errorMesg = ":SOVIET 481 " + client->getNickname() + " :Permission Denied - You are not a channel operator, beeetch\r\n";
                send(client_fd, errorMesg.c_str(), errorMesg.length(), 0);
            }
            else
            {
